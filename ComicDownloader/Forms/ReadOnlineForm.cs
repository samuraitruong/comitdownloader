...
namespace ComicDownoader.Forms
{
    ...
    /// </summary>
    public partial class ReadOnlineForm : Form
    {
        ...
        public ReadOnlineForm(string[] sFilePaths)
        {
            InitializeComponent();
            InitializeChildForm();
        ...
        private Bitmap GetImage(string sPath)
        {
            try
            {
                ...
                {
                    if (cached[sPath] != null)
                    {
                        using (Image oImage = Image.FromFile(((string)cached[sPath])))
                            return new Bitmap(oImage, oImage.Size);
                    }

                    var tempPath = Path.GetTempPath();
                    var savePath = downloader.DownloadPage(sPath, Path.GetRandomFileName(), tempPath, sPath);
                    cached[sPath] = savePath;

                    using (Image oImage = Image.FromFile((savePath)))
                        return new Bitmap(oImage, oImage.Size);
                }
        ...
        private void LoadNextImage(bool bHideWindow)
        {
            ...
            try
            {
                ...
                this.Invoke(new MethodInvoker(delegate() {
                    m_oImageForm.BackgroundImage = DrawingTools.CreateTextBitmap("Loading....",
        ...
        private void ReadOnlineForm_Resize(object sender, EventArgs e)
        {
